...
import { formatDate, getWeatherTextColor, degreesToDirection } from '@/lib/weather-utils';
...
const WeatherDetail = ({ 
  ...
  return (
    <div className={`${color} rounded-2xl p-3 flex flex-col items-center justify-center min-w-20`}>
...
const CurrentWeather = ({ data, isLoading }: CurrentWeatherProps) => {
  if (isLoading || !data) {
    return (
      <div className="bg-white/10 backdrop-blur-xl rounded-3xl p-5 min-h-[180px] animate-pulse">
  ...
  return (
    <div className="space-y-4">
      <div className="bg-blue-500/80 backdrop-blur-xl rounded-3xl p-5 animate-fade-in relative overflow-hidden">
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between z-10 relative">
          <div>
            ...
            <div className="text-white/70 text-sm mt-3">
              Feels like {Math.round(feelsLike)}°C • {windDirection} wind, {windSpeed} km/h
            </div>
          </div>
        ...
        <div className="absolute top-0 right-0 bottom-0 left-0 bg-gradient-to-br from-blue-400/30 to-blue-600/30 z-0"></div>
      </div>
