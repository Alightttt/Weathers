...
export const AnimatedWeatherIcon = ({ weatherCondition, size = 'medium' }: WeatherIconProps) => {
  ...
  if (condition.includes('cloud') && !condition.includes('rain') && !condition.includes('thunder')) {
    return (
      <div className={`weather-icon ${sizeClass[size]}`}>
        <Cloud className="w-full h-full text-gray-300 animate-float opacity-80" />
      </div>
    );
  }
  ...
  if (condition.includes('rain') || condition.includes('drizzle')) {
    return (
      <div className={`weather-icon ${sizeClass[size]}`}>
        <CloudRain className="w-full h-full text-blue-400 animate-pulse opacity-80" />
      </div>
    );
  }
  ...
  if (condition.includes('sun') || condition.includes('clear')) {
    return (
      <div className={`weather-icon ${sizeClass[size]}`}>
        <Sun className="w-full h-full text-amber-300 animate-spin-slow opacity-80" />
      </div>
    );
  }
  ...
  if (condition.includes('thunder') || condition.includes('storm')) {
    return (
      <div className={`weather-icon ${sizeClass[size]} relative`}>
        <Cloud className="w-full h-full text-gray-500 opacity-80" />
        <div className="absolute inset-0 flex items-center justify-center">
          <div className="w-1/2 h-1/2">
            <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 translate-y-0 w-1/3 h-1/2 bg-yellow-300 animate-lightning" style={{ clipPath: 'polygon(50% 0%, 0% 100%, 100% 100%)' }}></div>
          </div>
        </div>
      </div>
    );
  ...
  if (condition.includes('snow')) {
    return (
      <div className={`weather-icon ${sizeClass[size]}`}>
        <Snowflake className="w-full h-full text-blue-100 animate-float opacity-80" />
      </div>
    );
  }
  ...
  if (condition.includes('fog') || condition.includes('mist')) {
    return (
      <div className={`weather-icon ${sizeClass[size]}`}>
        <CloudFog className="w-full h-full text-gray-400 animate-pulse opacity-80" />
      </div>
    );
  }
  ...
  return (
    <div className={`weather-icon ${sizeClass[size]}`}>
      <CloudSun className="w-full h-full text-gray-300 opacity-80" />
    </div>
  );
};
